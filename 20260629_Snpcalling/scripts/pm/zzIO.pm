...
my $bgzip = '/share/appspace_data/shared_groups/caas_zhengxm04_share_software/micromamba/envs/snp_calling_py311/bin/bgzip';
my $pigz = '/usr/bin/pigz';
my $crabz = '';
...
sub open_in_fh($;$) {
    ...
    if ($in eq '-') {
        ...
        open ($fh,"zcat $in|");
        if ($parallel>1) {
            open ($fh,"$bgzip --threads $parallel --decompress --stdout $in |");
        } else {
            open ($fh,"zcat $in|");
        }
        return $fh;
